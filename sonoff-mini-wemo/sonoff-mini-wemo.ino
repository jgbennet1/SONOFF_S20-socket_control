...
void setup() {
  ...
  if(wifiConnected){
    ...
    pinMode(statusLED, OUTPUT); // turn on the status LED if connected to WiFi
...
// Function to handle toggling power with the button on the device
void togglePwr() {
  // Serial.println("button pushed");
  if(digitalRead(relayPin) != 0) { // checks for relay state
    turnOffRelay();
  }
  else {
    turnOnRelay();
